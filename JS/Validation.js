...
ValidateRequiredFieldBlog = function (ControlID, Errormesage) {
    var IsError = "";
    if (ControlID.value.length == 0) {
        ControlID.style.background = '#FCFAA9';
        if (Focus == "No")
...
validateEmailblog = function (ControlID) {
    var IsError = "";
    var tfld = trim(ControlID.value);                        // value of field with #E3E9E9space trimmed off
    var emailFilter = /^[^@]+@[^@.]+\.[^@]*\w\w$/;
    var illegalChars = /[\(\)\<\>\,\;\:\\\"\[\]]/;
    ...
    if (ControlID.value == "") {
        ...
        SetFocusOnValidateField(ControlID);
        ControlID.style.background = '#FCFAA9';
        IsError = "Please enter email address!<br>";

    } else if (!emailFilter.test(tfld)) {              //test email for illegal characters

        SetFocusOnValidateField(ControlID);
        ControlID.style.background = '#FCFAA9';
        IsError = "Please enter valid email address! e.g tpss@gmail.com <br>";

    } else if (ControlID.value.match(illegalChars)) {

        SetFocusOnValidateField(ControlID);
        ControlID.style.background = '#FCFAA9';
        IsError = "The email address contains illegal characters!<br>";
    } else {
        //ControlID.style.background = '#E3E9E9';
