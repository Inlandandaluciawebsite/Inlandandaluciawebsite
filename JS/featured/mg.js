...
Mg.prototype.refreshItems = function() {
	...
	if(populateHrefAnchors){
		...
		for(var i=0, len=this.items.length; i<len; i++){
			this.itemsLink.push({href:this.items[i].getAttribute("href"), title:this.items[i].getAttribute("title")});
		}
	}
	this.initEvents();
};
...
Mg.prototype.init = function(isAnchor) {
	for(var i=0, len=this.events.length; i<len; i++){
		...
		// setState for init
		for(var z=0, len=thisEvnt.activated.length; z<len; z++){
			/*if(isAnchor){
 ...
 * for featured with num = 1, less = 2, plus = 2, max = 5, cycle = true the return array is [5, 0, 1, 2, 3]
 ...
 * for featured with num = 6, less = 2, plus = 2, max = 7, cycle = false the return array is [5, 6, 7, null, null]
...
if(!!(window.history && history.pushState)){
	...
	history.onpushstate = function(state, title, url){
		// only when it's a link click that changes the url
		if(!state){
...
Mg.prototype.makeAnchor = function(value, key, eventType){
	var evnt = this[key];
	if(evnt.anchorize == "useLinkHash" && window.mg_HashEvntsArr){ // second check is browser support
		// if we have useLinkHash we can have only an event on anchor
		if(eventType == "init"){
	...
	}else if(evnt.anchorize == "useLinkUrl" && window.mg_UrlEvntsArr){ // second check is browser support
		// if we have useLinkHash we can have only an event on anchor
		if(eventType == "init"){
			// if init we see if the anchor is on the items list
			var arr = this.itemsLink;
			...
			// else we set the url
			history.pushState(true, null, this.itemsLink[value].href);
			this.setDocumentTitle(this.itemsLink[value].title);
		}
	}else if(evnt.anchorize == "useAnchor" && window.mg_AnchorEvntsArr){ // second check is browser support
...
Mg.prototype.addScrollDrag = function(evnt, itemIn, itemOut, min, max, vertical, scrollDragStr, concurrent){
	...
	function itemInOnMouseMove(e){
		...
		if(Math.abs(dist) > 10 && Math.abs(distOther) < 50){
	...
	function itemInOnMouseUp(e){
		...
		if(target.clicked){
			...
			if(Math.abs(dist)>25){ // if is movement
				...
				var f = false;
				if(!target.isScroll){
					if(tdist < 500 && Math.abs(dist) < target.swipeDist/2 && Math.abs(distOther) < 100){
