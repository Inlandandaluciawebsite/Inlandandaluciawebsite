...
Partial Class ManageLawyer
    ...
    Private Sub bindadmin()
        ...
        sqlSearch(3) = New SqlParameter("@TypeId", 2)
    ...
    Protected Sub grdAdmin_RowCommand(sender As Object, e As GridViewCommandEventArgs)
        If e.CommandName = "editadmin" Then
            Response.Redirect("Addlawyer.aspx?ContactId=" + e.CommandArgument.ToString())
        End If
    ...
    End Sub
    Public Function AllPartners_By_Email(ByVal Contact_Email As String) As String
        Dim sqlbrokerpartners As SqlParameter() = New SqlParameter(2) {}
        sqlbrokerpartners(0) = New SqlParameter("@Contact_Email", Contact_Email)
        sqlbrokerpartners(1) = New SqlParameter("@Contact_Type_Id", 2)
        Dim dt As DataTable = SqlHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings("con").ConnectionString, CommandType.StoredProcedure, "USP_Get_CommaSeperatedPartners_By_Email", sqlbrokerpartners).Tables(0)
        If dt.Rows.Count > 0 Then
