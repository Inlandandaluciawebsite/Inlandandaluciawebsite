...
Partial Class AddPartner
        ...
        If Not IsPostBack Then
            ...
            If Request.QueryString.HasKeys() Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    If drpType.SelectedItem.Text = "Partner/Franchisee" Then
                        chkArchived.Checked = Convert.ToBoolean(dt.Rows(0)("Contact_Archived"))
                        btnAction.Visible = True
                        btnClintHistory.Visible = True
                        btnClintHistory.Text = "PARTNER HISTORY"
                    Else
                        btnAction.Visible = True
                        btnClintHistory.Visible = True
                        chkArchived.Checked = False
                        chkArchived.Text = "Approve As Partner ? "
                        lblHeading.Text = "Manage Franchise Request"
                    End If

                    'Binding Buyer Source
                    If Convert.ToInt32(dt.Rows(0)("Source")) <> 0 Then
                        DropDownListSource.SelectedValue = dt.Rows(0)("source").ToString()
                    End If
                End If
            End If
        End If
    ...
    Protected Sub btnSubmit_Click(sender As Object, e As EventArgs)
        ...
        Try
            'Insert into franchise_source table
            Dim sqlFranchiseSource As SqlParameter() = New SqlParameter(1) {}
            sqlFranchiseSource(0) = New SqlParameter("@Franchise_Id", ContactId)
            sqlFranchiseSource(1) = New SqlParameter("@Buyer_Source_Id", DropDownListSource.SelectedValue)
            SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings("con").ConnectionString, CommandType.StoredProcedure, "USP_Franchise_Source_Insert", sqlFranchiseSource)
    ...
    Protected Sub btnUpdate_Click(sender As Object, e As EventArgs)
        ...
        If chkArchived.Checked And chkArchived.Text = "Approve As Partner ? " Then
            sql(0) = New SqlParameter("@Contact_Type_ID", 3)
        Else
            sql(0) = New SqlParameter("@Contact_Type_ID", Convert.ToInt32(drpType.SelectedValue))
        End If
        ...
        sql(9) = New SqlParameter("@Contact_Language_ID", Convert.ToInt32(drpspoken.SelectedValue))
        sql(10) = New SqlParameter("@Contact_Partner_ID", 0)
        If chkArchived.Text = "Approve As Partner ? " Then
            sql(11) = New SqlParameter("@Contact_Archived", False)
        Else
            sql(11) = New SqlParameter("@Contact_Archived", chkArchived.Checked)
        End If
        ...
        Try
            Dim sqlFranchiseSource As SqlParameter() = New SqlParameter(1) {}
            sqlFranchiseSource(0) = New SqlParameter("@Franchise_Id", id)
            sqlFranchiseSource(1) = New SqlParameter("@Buyer_Source_Id", DropDownListSource.SelectedValue)
            SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings("con").ConnectionString, CommandType.StoredProcedure, "USP_Franchise_Source_Insert", sqlFranchiseSource)
            ' Response.Redirect("ManageAgents.aspx")
    ...
    Protected Sub btnAction_Click(sender As Object, e As EventArgs)
        If drpType.SelectedItem.Text = "Partner/Franchisee" Then
            Response.Redirect("PartnerActionAgenda.aspx?PartnerId=" & Request.QueryString("ContactId").ToString())
        Else
            Response.Redirect("FranchiseActionAgenda.aspx?FranchiseId=" & Request.QueryString("ContactId").ToString())
        End If
    End Sub
    Protected Sub btnClintHistory_Click(sender As Object, e As EventArgs)
        If drpType.SelectedItem.Text = "Partner/Franchisee" Then
            Response.Redirect("PartnerHistory.aspx?PartnerId=" & Request.QueryString("ContactId").ToString())
        Else
            Response.Redirect("FranchiseHistory.aspx?FranchiseId=" & Request.QueryString("ContactId").ToString())
        End If
    End Sub
End Class
