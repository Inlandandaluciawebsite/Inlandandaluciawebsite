...
Partial Class AddUser
        ...
        If Not IsPostBack Then
            ...
            If Request.QueryString.HasKeys() Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    CheckBoxAdmin.Checked = dt.Rows(0)("Contact_Admin").ToString()
                    ...
                    'If dt.Rows(0)("Contact_Partner_ID").ToString() > 0 Then

                    '    drppartner.SelectedValue = (dt.Rows(0)("Contact_Partner_ID").ToString())
                    'End If
                    ' Tidy
                    CDataAccess = Nothing
                    ...
                    sqlBrokerPartner(1) = New SqlParameter("@Contact_Type_Id", 4)
    ...
    Protected Sub btnSubmit_Click(sender As Object, e As EventArgs)
        ...
        For nPartnerIndex = 0 To chklstPartner.Items.Count - 1
            ...
            If chklstPartner.Items(nPartnerIndex).Selected Then
                ...
                sql(15) = New SqlParameter("@Contact_Admin", CheckBoxAdmin.Checked)
                ...
                Else
                    ...
                    sql(1) = New SqlParameter("@Contact_Name", txtName.Text & "-" & chklstPartner.Items(nPartnerIndex).Text.Substring(0, 2))
                End If

                Dim ContactId As Integer = Convert.ToInt32(SqlHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings("con").ConnectionString, CommandType.StoredProcedure, "Usp_CONTACT_Insert_user", sql).Tables(0).Rows(0)("ContactId").ToString())
        ...
        If firstContactId <> 0 Then
            Response.Redirect("AddUser.aspx?ContactId=" & firstContactId & "&ContactSaved=Yes")
        End If
        ...
        For nPartnerIndex = 0 To chklstPartner.Items.Count - 1
            ...
            If chklstPartner.Items(nPartnerIndex).Selected Then
                ...
                If dsCheckPartnerExists.Rows.Count > 0 Then
                    ...
                    sql(16) = New SqlParameter("@Contact_Admin", CheckBoxAdmin.Checked)
                    If drpMainContact.SelectedItem.Text = chklstPartner.Items(nPartnerIndex).Text Then
                        sql(17) = New SqlParameter("@IsMainContact", True)
                        sql(1) = New SqlParameter("@Contact_Name", txtName.Text)
                    Else
                        sql(17) = New SqlParameter("@IsMainContact", False)
                        sql(1) = New SqlParameter("@Contact_Name", txtName.Text & "-" & chklstPartner.Items(nPartnerIndex).Text.Substring(0, 2))
                    End If
                    SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings("con").ConnectionString, CommandType.StoredProcedure, "Usp_CONTACT_Update_user", sql).ToString()
                Else
                    Dim sql As SqlParameter() = New SqlParameter(17) {}
                    sql(0) = New SqlParameter("@Contact_Type_ID", Convert.ToInt32(drpType.SelectedValue))
                    sql(2) = New SqlParameter("@Contact_Registration_Number", txtRegistrationNo.Text)
            ...
            Else
                ...
                If dsCheckPartnerExists.Rows.Count > 0 Then
                    id = Convert.ToInt32(dsCheckPartnerExists.Rows(0)("Contact_Id").ToString())
                    Dim sql As SqlParameter() = New SqlParameter(18) {}
                    sql(0) = New SqlParameter("@Contact_Type_ID", Convert.ToInt32(drpType.SelectedValue))
    ...
    '    sql(8) = New SqlParameter("@Contact_Notes", txtNotes.Text)
    '    sql(9) = New SqlParameter("@Contact_Language_ID", Convert.ToInt32(drpspoken.SelectedValue))
    '    sql(10) = New SqlParameter("@Contact_Partner_ID", Convert.ToInt32(chklstPartner.SelectedValue))
    '    sql(11) = New SqlParameter("@Contact_Archived", chkArchived.Checked)
    '    sql(12) = New SqlParameter("@Contact_ID", id)
    '    sql(13) = New SqlParameter("@Contact_Login", txtlogin.Text)
    '    sql(14) = New SqlParameter("@Contact_Password", txtpassword.Text)
