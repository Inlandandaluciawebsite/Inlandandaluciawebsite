...
Partial Class ClientTours
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        Else
            ...
            DropDownListViewDateDay.SelectedValue = DateTime.Now.Day
            DropDownListViewDateYear.SelectedValue = DateTime.Now.Year
            DropDownListViewDateMonth.SelectedIndex = DateTime.Now.Month - 1
            ...
            DropDownListMeetingTimeHour.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListMeetingTimeMinute.SelectedValue = "00"
            DropDownListTimeHour1.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute1.SelectedValue = "00"
            DropDownListTimeHour2.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute2.SelectedValue = "00"
            DropDownListTimeHour3.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute3.SelectedValue = "00"
            DropDownListTimeHour4.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute4.SelectedValue = "00"
            DropDownListTimeHour5.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute5.SelectedValue = "00"
            DropDownListTimeHour6.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute6.SelectedValue = "00"
            DropDownListTimeHour7.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute7.SelectedValue = "00"
            DropDownListTimeHour8.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute8.SelectedValue = "00"
            DropDownListTimeHour9.SelectedValue = DateTime.Now.Hour.ToString()
            DropDownListTimeMinute9.SelectedValue = "00"
            DropDownListTimeHour10.SelectedValue = DateTime.Now.Hour.ToString()
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            If Not Session("AdminCreateTourBuyerID") Is Nothing Then
                ...
                ' Init Control
                InitBuyer(Convert.ToInt32(Session("AdminCreateTourBuyerID")))
            ...
            If Not Session("AdminClientTourPropertiesSelected") Is Nothing Then
                ...
                ' Init Control
                'InitSelectedProperties(Session("AdminClientTourPropertiesSelected"))
                LoadTour()
            Else
                LoadTour()
            End If
            ...
            'If Not Request.QueryString("TourId") = "" Then
            '    Session("ClientTourID") = Request.QueryString("TourId").ToString()
            'End If
            ...
            'Check if tour already have feedback then disable SAVE button

            Dim sqlCheckTourFeedback As SqlParameter() = New SqlParameter(1) {}
            sqlCheckTourFeedback(0) = New SqlParameter("@Tour_Id", Convert.ToInt32(Session("ClientTourID")))
            Dim dtCheckTourFeedback As DataTable = SqlHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings("IA").ConnectionString, CommandType.StoredProcedure, "USP_CheckTourFeedback_By_TourId", sqlCheckTourFeedback).Tables(0)
            If dtCheckTourFeedback.Rows.Count > 0 Then
                ButtonSave.Visible = False
                'ButtonFeedback.Visible = False
            End If
        End If
    ...
    Private Sub UpdatePropertyDetails(ByVal ddl As DropDownList, ByVal nPropertyID As Integer, ByRef lblpatner As Label, ByRef lblName As Label, ByRef lblAddress As Label, ByRef lblTown As Label, ByRef lblContactNo As Label, ByRef lblPrice As Label, ByRef ddlHour As DropDownList, ByRef lblSeparator As Label, ByRef ddlMinute As DropDownList, ByRef lblKey As Label)
        ...
        If dtDataTable.Rows.Count > 0 Then
            ...
            lblName.Text = dtDataTable.Rows(0).Item("Name")
            lblAddress.Text = dtDataTable.Rows(0).Item("Address")
            lblTown.Text = dtDataTable.Rows(0).Item("Town")
            lblContactNo.Text = dtDataTable.Rows(0).Item("Telephone") & " / " & dtDataTable.Rows(0).Item("Mobile")
    ...
    Protected Sub ButtonSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonSave.Click
        ...
        'Check If Property Time is less than meeting time

        Dim propertyTimeHour As Int32 = Convert.ToInt32(DropDownListMeetingTimeHour.SelectedValue)
        Dim propertyTimeMinute As Int32 = Convert.ToInt32(DropDownListMeetingTimeMinute.SelectedValue)
        Dim isLess As Int32 = 0

        If DropDownListTimeHour1.Visible = True And Convert.ToInt32(DropDownListTimeHour1.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour2.Visible = True And Convert.ToInt32(DropDownListTimeHour2.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour3.Visible = True And Convert.ToInt32(DropDownListTimeHour3.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour4.Visible = True And Convert.ToInt32(DropDownListTimeHour4.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour5.Visible = True And Convert.ToInt32(DropDownListTimeHour5.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour6.Visible = True And Convert.ToInt32(DropDownListTimeHour6.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour7.Visible = True And Convert.ToInt32(DropDownListTimeHour7.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour8.Visible = True And Convert.ToInt32(DropDownListTimeHour8.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour9.Visible = True And Convert.ToInt32(DropDownListTimeHour9.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If DropDownListTimeHour10.Visible = True And Convert.ToInt32(DropDownListTimeHour10.SelectedValue) < propertyTimeHour Then
            isLess = 1
        End If

        If isLess = 1 Then
            lblMessage.Visible = True
            lblMessage.Text = "Please check you Viewing time, can't be set after property visit time."
        ...
        CClientTour.VirtualTour = chkVirtualTour.Checked
        ...
        'Update Buyer Status for tour OR virtual tour
        ...
        ' Local Vars
        Dim szSQL As String = ""
        szSQL = "update buyer set Buyer_Status_ID=" & Buyer_Status_Id & " where Buyer_ID=" & CClientTour.BuyerID
        CDataAccess.Execute(szSQL)
        CDataAccess = Nothing
        szSQL = ""

        Dim sql As SqlParameter() = New SqlParameter(7) {}
        sql(0) = New SqlParameter("@Buyer_Status", Buyer_Status_Id)
        sql(1) = New SqlParameter("@Buyer_ID", CClientTour.BuyerID)
        sql(2) = New SqlParameter("@Action_Date", DateTime.Now.ToString())
        sql(3) = New SqlParameter("@History_Text", history_text)
        sql(4) = New SqlParameter("@Modified_By", Convert.ToInt32(Session("ContactID")))
        sql(5) = New SqlParameter("@Archived", 0)
        sql(6) = New SqlParameter("@Action_Status", "Information Only")
        ...
        ' Reload the Tour
        LoadTour()
        btnCreateNewTour.Visible = True
        Session.Remove("AdminCreateTourBuyerID")
    End Sub
    ...
    Public Sub LoadTour()
        ...
        If CClientTour.BuyerID <> Convert.ToInt32(Session("AdminCreateTourBuyerID")) Then
            If Not Session("AdminCreateTourBuyerID") Is Nothing Then
                DropDownListBuyer.SelectedValue = Convert.ToInt32(Session("AdminCreateTourBuyerID"))
            Else
        ...
        chkVirtualTour.Checked = CClientTour.VirtualTour
        If CClientTour.BuyerID = 0 And CClientTour.TourWithID = 0 Then
            DropDownListViewDateYear.SelectedValue = DateTime.Now.Year
            DropDownListViewDateMonth.SelectedIndex = DateTime.Now.Month - 1
            PopulateDaysOfMonth()
            DropDownListViewDateDay.SelectedValue = DateTime.Now.Day
            DropDownListMeetingTimeHour.SelectedValue = Format(DateTime.Now.Hour, "00")
        ...
        'Check if viewing date and viewing time is past then show feedback button
        Dim dtCurrentDate As DateTime = DateTime.Now
        'create viewing date time example 2008-03-09 16:05:07.123
        Dim dtViewingYear As Int32 = Convert.ToInt32(DropDownListViewDateYear.SelectedValue)
        Dim dtViewingMonth As Int32 = Convert.ToInt32(DropDownListViewDateMonth.SelectedIndex) + 1
        Dim dtViewingDay As Int32 = Convert.ToInt32(DropDownListViewDateDay.SelectedValue)
        Dim dtViewingHour As Int32 = Convert.ToInt32(DropDownListTimeHour1.SelectedValue)
        Dim dtViewingMinute As Int32 = Convert.ToInt32(DropDownListTimeMinute1.SelectedValue)
        Dim dtViewingDate As New DateTime(dtViewingYear, dtViewingMonth, dtViewingDay, dtViewingHour, dtViewingMinute, 7, 123)

        If dtCurrentDate > dtViewingDate And CClientTour.TourID <> 0 Then
            ButtonFeedback.Visible = True
        Else
            ButtonFeedback.Visible = False
        End If
    ...
    Protected Sub ButtonFeedback_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonFeedback.Click
        ...
        Session("ClientTourID") = Nothing
        Response.Redirect("ClientTourFeedback.aspx")
        ' Raise Event
        'RaiseEvent Feedback()

    End Sub
    Protected Sub btnCreateNewTour_Click(sender As Object, e As EventArgs)
        Session("AdminClientTourPropertiesSelected") = Nothing
        Session("ClientTourID") = Nothing
